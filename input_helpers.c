 ...
 * otherwiise - The exit value of the last executed command.
 */
int handle_args(int *exe_ret)
{
	char *line = NULL;
...
/**
 * _check_arg - Checks if there are any leading   '&&',';;',';', or '||'.
...
int _check_arg(char **args)
{
	...
	for (i = 0; args[i]; i++)
	{
		...
		{
			if (i == 0 || cur[1] == ';')
				return (_error_creat(&args[i], 2));
			nex = args[i + 1];
			if (nex && (nex[0] == ';' || nex[0] == '&' || nex[0] == '|'))
				return (_error_creat(&args[i + 1], 2));
		}
	}
	return (0);
}
